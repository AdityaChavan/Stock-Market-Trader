...
class DTLearner(object):
  ...
  def addEvidence(self,Xtrain,Ytrain):
    ...
    if (flag1 or flag2 ):#its a leaf!
      stat=self.inc_no()
      #print np.array([stat,'leaf',np.mean(Ytrain),np.nan,np.nan])
      ...
      with warnings.catch_warnings():
        ...
        max_corr_index= np.nanargmax(np.absolute([np.corrcoef(Xtrain[:,i],Ytrain)[0,1] for i in range(np.size(Xtrain[0,:])) ]))
